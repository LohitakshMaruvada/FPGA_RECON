...
`define AUTOTB_TVIN_j_out_wrapc  "../tv/rtldatafile/rtl.MAC.autotvin_j.dat"
`define AUTOTB_TVOUT_ap_return  "../tv/cdatafile/c.MAC.autotvout_ap_return.dat"
`define AUTOTB_TVOUT_ap_return_out_wrapc  "../tv/rtldatafile/rtl.MAC.autotvout_ap_return.dat"
module `AUTOTB_TOP;
...
// The signal of port j
reg [31: 0] AESL_REG_j = 0;
assign j = AESL_REG_j;
initial begin : read_file_process_j
    integer fp;
    integer err;
    integer ret;
    integer proc_rand;
    ...
    transaction_idx = 0;
    wait(AESL_reset === 0);
    fp = $fopen(`AUTOTB_TVIN_j,"r");
    if(fp == 0) begin       // Failed to open file
        $display("Failed to open file \"%s\"!", `AUTOTB_TVIN_j);
        $display("ERROR: Simulation using HLS TB failed.");
        $finish;
    end
    read_token(fp, token);
    ...
    while (token != "[[[/runtime]]]") begin
        ...
        if(token != "[[/transaction]]") begin
            ret = $sscanf(token, "0x%x", AESL_REG_j);
              if (ret != 1) begin
                  $display("Failed to parse token!");
                $display("ERROR: Simulation using HLS TB failed.");
                  $finish;
